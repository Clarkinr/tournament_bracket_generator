...
def gen_num_games(team_list):
    """
    Shuffle the team list for the new round in order
    to have randomly assigned games
...
def advance_round(match_split):
    ...
    While loop based on CI's Love sandwiches project
    """
    ...
    for i in range(0, len(match_split)):
        ...
        print(f"{match_split[i][0]} VS {match_split[i][1]}\n")
        print("Enter 0 for all 'BYE' goals and 1 for the oposition\n")

        while True:
            result_1 = input(f"{match_split[i][0]}'s Goals:\n\n>")
            result_2 = input(f"{match_split[i][1]}'s Goals:\n\n>")
        ...
        if next_game != "y":
            input("Ok, press the return key when you are ready\n>")
            print("------------------------------------------------\n")
...
def validate_input(value):
    ...
    Code is adapted from CI's Love Sanwiches project
    """
    try:
        ...
        print("------------------------------------------------\n")
        print(f"\n ERROR: Please only use integer values: {e}\n")
        print("------------------------------------------------\n")
        return False
